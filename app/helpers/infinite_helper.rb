module InfiniteHelper
  def infinite_append(containerSelector, render_options)
    ...
    if collection.next_page
      html += "$('.infinite-pagination').replaceWith('" + j(will_paginate(collection, render_options.except(:partial, :collection).merge({renderer: WillPaginateInfinite::InfinitePagination}) )) + "');"
    else
